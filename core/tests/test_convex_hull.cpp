...
TEST_CASE("Convex hull test") {
  std::vector<std::vector<Treatment>> treatment_arrays = {
    {
      Treatment(1, 10.0, 1.0),
      Treatment(2, 15.0, 2.0),
      Treatment(3, 20.0, 3.0)
    },
  ...
  CHECK(treatment_arrays[2].size() == 3);
  ...
  CHECK(treatment_arrays[2][1].id == 2);
  CHECK(treatment_arrays[2][2].id == 3);
}
